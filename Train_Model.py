...
Trains the CelebA-HQ diffusion model. Dataset needs to be prepared first.
"""
...
# ------------------------
def train():
    data_dir = '/data5/accounts/marsh/.cache/kagglehub/datasets/spandan2/cats-faces-64x64-for-generative-models/versions/1/cats/cats/'#"./celeba_hq_prepared"  # Replace with your path
    save_dir = "./vp_diffusion_outputs" # Replace with your path
    ...
    for epoch in range(epochs):
        ...
        torch.save(model.state_dict(), os.path.join(save_dir, f"unet_cat_epoch_{epoch+1}.pt"))

        # Save sample images
        samples = sample(model, scheduler, device, num_steps=num_timesteps, img_size=image_size, batch_size=4)
        save_image(samples, os.path.join(save_dir, f"samples_cat_epoch_{epoch+1}.png"), nrow=2)
