...
Updates to expected primitive multiple times from a random initial datapoint.
"""
...
def main():
    ...
    eps = 1.0
    N = 16
    max_updates = 2   # <-- limit number of iterations
    ...
    # -------------------------
    # Iterative expected-primitive flow
    # -------------------------
    for it in range(max_updates):
        ...
        with torch.no_grad():
            img = Phi_fn(z)
            img_vis = (img[0] * 0.5 + 0.5).clamp(0, 1).cpu()
            images.append(img_vis)

        # Compute expected primitive
        z_star = expected_primitive(
            z0=z,
            Z=z + eps * torch.randn(N, 3, 64, 64, device=device),
            Phi_fn=Phi_fn,
            log_map_fn=log_map_fn,
            exp_map_fn=exp_map_fn,
            eps=eps,
            max_iter=2,   # keep this small for stability
            lr=0.01
        )

        # Expected primitive update
        with torch.no_grad():
            z = z_star

    # -------------------------
    # Plot all steps sequentially
    # -------------------------
