...
def confidence_metric(
    ...
    eps: float = 1.0,            # radius for the ε-ball
    ...
    # -------------------------
    # 1. Construct ε-ball in latent space and map to image space
    # -------------------------
    def sample_ball(z0, eps, N):
        ...
        return z0 + (eps * r * u).view(N, *shape[1:])
    ...
    geodesic_dists = []
    for i in range(N):
        log_i = log_map_fn(y_star, Y[i:i+1])
    ...
    log_center = log_map_fn(y_star, Phi_fn(z))
    ...
    # -------------------------
    C = torch.log(torch.tensor(var_R)) + dist_center**2 / var_R
...
def main():
    ...
    lam = 1000.0
    beta = 0.0
    eps = 1.0      # epsilon value for the ball
    N = 128         # Number of samples for Monte Carlo
    num_samples = 1  # Number of initial random samples
    ...
    def log_map_fn(x, y):
        return log_map_shooting(
            y=x, y_target=y,
            model=model, max_iters=2000, scheduler=scheduler,
    ...
    for i in range(num_samples):
        ...
        # 2. Sample points Z in the ball
        Z = sample_ball(z0, eps, N)
        print(f"Generated {Z.shape[0]} samples in ε-ball for sample {i+1}.")
