...
# ------------------------
@torch.no_grad()
def levi_civita_exp_map(y, v, model, scheduler, t_idx, lambda=1.0, n_substeps=8):
    y_curr = y.clone()
    dx = v / n_substeps
    for _ in range(n_substeps):
        ...
        factor = lambda * s_dot_dx / (1 + lambda * s_norm2)
...
def levi_civita_log_map(Y, Y_target, model, scheduler, t_idx,
                              lambda=1.0, n_substeps_schedule=[1,2,4,8],
    ...
    for n_substeps in n_substeps_schedule:
        ...
        for i in range(n_iters):
            ...
            Y_pred = levi_civita_exp_map(Y, v, model, scheduler, t_idx,
                                         lambda=lambda, n_substeps=n_substeps)
...
def main():
    ...
    checkpoint_path = "./vp_diffusion_outputs/unet_epoch_2000.pt" # Replace with your filepath (can be Animal Faces)
    ...
    log_map_fn = lambda x, y: levi_civita_log_map(Y=x, Y_target=y, model=model, scheduler=scheduler, t_idx=t_idx, lambda=lambda)
    exp_map_fn = lambda x, v: levi_civita_exp_map(y=x, v=v, model=model, scheduler=scheduler, t_idx=t_idx, lambda=lambda)
