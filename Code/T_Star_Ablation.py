#!/usr/bin/env python3
"""Paper t* sensitivity ablation for High-Confidence Curves."""
...
def parse_args():
    p = argparse.ArgumentParser(description="HCC t* ablation")
    ...
    p.add_argument("--tstar-values", nargs="+", type=int,
...
def main():
    ...
    for tstar in args.tstar_values:
        ...
        run = run_hcc_from_latents(
            model, scheduler, z_pair, cfg, seed=seed, proxy_t=int(tstar)
        )
        final_x = decode_display(model, scheduler, run["final_z"], cfg)
        rows.append((rf"$t^*={tstar}$", final_x))
        ...
        if accepted:
            ax.plot(range(1, len(accepted) + 1), accepted, label=rf"$t^*={tstar}$")

    save_curve_grid(rows, out_dir / "tstar_ablation.png")
    ax.set_xlabel("HCC refinement iteration")
    ax.set_ylabel("Mean accepted frozen local objective")
    ax.set_yscale("log")
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "tstar_objective_history.png", dpi=220, bbox_inches="tight")
    plt.close(fig)
    save_json(results, out_dir / "tstar_ablation.json")
