...
# Levi-Civita Exp Map
# ------------------------
def levi_civita_exp_map(x, v, model, scheduler, t_idx, =1.0, x_ind=None, n_steps=10, n_steps_int=10):
    """
...
# ------------------------
@torch.no_grad()
def log_map_shooting(y, y_target, model, scheduler, t_idx, lambda=1e6, max_iters=1000, lr=0.1, n_substeps_schedule=[1, 2, 4, 8],):
    y = y.detach()
...
def main():
    ...
    checkpoint_path = "./vp_diffusion_outputs/unet_epoch_2000.pt" # Replace with your filepath (can also load the animal faces model
    ...
    for lambda in lambda_values:
        print(f"Computing bidirectional geodesic for lambda={lambda}...")
        v_forward = log_map_shooting(xA2, xB2, model, scheduler, t_idx, lambda=lambda)

        geodesic = []
        for i in range(n_geo_steps):
            s = i / (n_geo_steps - 1)
            y_s = levi_civita_exp_map(xA2, s * v_forward, model, scheduler, t_idx, lambda=lambda)
        ...
        all_geodesics[lambda] = geodesic
        print(f"Geodesic complete for lambda={lambda}.")
    ...
    for row_idx, lambda in enumerate(lambda_values):
        geodesic = all_geodesics[lambda]
        y_offset = row_idx * (height + label_height)
        if font:
            draw = ImageDraw.Draw(final_image)
            draw.text((0, y_offset), f"lambda={lambda}", fill=(255,0,0), font=font)
