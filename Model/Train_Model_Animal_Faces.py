"""
Trains a diffusion model for the Animal Faces dataset and saves checkpoints.
"""
...
class AnimalFacesDataset(Dataset):
    def __init__(self, root_dir, image_size=64):
        ...
        # Recursively collect all images from subfolders
        self.image_paths = []
...
class VPScheduler:
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        sqrt_alpha = torch.sqrt(alphas_cumprod_t).view(-1, 1, 1, 1)
        sqrt_one_minus = torch.sqrt(1 - alphas_cumprod_t).view(-1, 1, 1, 1)
...
# ------------------------
def train():
    data_dir = "./animal-faces/versions/1" # Replace with your path if your dataset is saved differently
    ...
    dataset = AnimalFacesDataset(data_dir, image_size=image_size)
    ...
    for epoch in range(epochs):
        ...
        torch.save(model.state_dict(), os.path.join(save_dir, f"unet_animal_epoch_{epoch+1}.pt"))

        samples = sample(model, scheduler, device, num_steps=num_timesteps, img_size=image_size, batch_size=4)
        save_image(samples, os.path.join(save_dir, f"samples_animal_epoch_{epoch+1}.png"), nrow=2)
